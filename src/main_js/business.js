document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculerPret(montant, dureeAnnees, salaire) {
        // Taux de base pour busiiiness : 6%
        let tauxBase = 6;

        // Ajustement du taux en fonction de la duree
        // Pour chaque annee au-delà de 15 ans, on ajoute 0.1% au taux
        let tauxFinal = tauxBase + (dureeAnnees > 15 ? (dureeAnnees - 15) * 0.1 : 0);
        tauxFinal = Math.min(tauxFinal, 8.0); // Taux maximum de 8%

        // Conversion en taux mensuel
        const tauxMensuel = tauxFinal / 100 / 12;
        const nbMois = dureeAnnees * 12;

        // Calcul de la mensualite (formule standard)
        const mensualite = montant * tauxMensuel * Math.pow(1 + tauxMensuel, nbMois) / (Math.pow(1 + tauxMensuel, nbMois) - 1);
    ...
    form.addEventListener('submit', function (e) {
        ...
        const resultats = calculerPret(montant, dureeAnnees, salaire);

        // Verification si le pret est accessible (mensualite < 40% du salaire)
        const pretAccessible = salaire === 0 || resultats.pourcentageSalaire <= 40;

        // Generation du contenu des resultats
        let html = '';
        ...
        if (pretAccessible) {
            ...
            html = `
                        <div class="text-center mb-8">
                            <h1 class="text-3xl font-bold text-red-600 mb-2">Pret Non Accessible !</h1>
                            <p class="text-lg text-gray-700">Votre mensualite represente <span class="font-bold text-red-600">${resultats.pourcentageSalaire.toFixed(1)}%</span> de votre salaire</p>
                        </div>
        ...
        // Affichage de la section resultats
